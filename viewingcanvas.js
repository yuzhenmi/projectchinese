function ViewingCanvas(canvas, changeword, changeset, changepage) {
      ...
      if (Math.sqrt(Math.pow(this.pos.x - this.lastPos.x, 2) + Math.pow(this.pos.y - this.lastPos.y, 2)) < 20) {  //tap
        if (this.selected < 0) {
          ...
          if (temp < this.svgids.length) {
            this.selected = temp;
            this.update();
            this.changeset(this.svgids[this.selected]);
          }
        }
        else {
...
ViewingCanvas.prototype.getCharacterId = function() {
	return this.svgids[this.selected][this.innerselected];
}
